...
public class ImageConverter 
{
    ...
    */
	protected File processImage( File input, File output, String targetFormat, float quality ) throws IOException, IllegalArgumentException
    {
        ...
        if ((targetFormat.equals(PNG) || targetFormat.equals(BMP)) || (targetFormat.equals(JPEG) && quality==1.0f)){

        	outputFile=writeImage(loadImage(input), input, output, targetFormat);
        }
        if (targetFormat.equals(JPEG) && (quality!=1.0f)){
        	System.out.println("Hello I'm here at JPEG");
        	outputFile=writeJpegImage(loadImage(input), input, output, quality);
        }
	...
	/**
	 * Loads the input image and converts it to a BufferedImage, so it can be processed in Java
	 * @param input Image-Path/-File
	 * @return Input Image as an BufferedImage
	 */
	 ...
	 * @return
	 */
	public File writeImage(BufferedImage input, File inputName, File output, String targetFormat){
		File f = null;
		try{
			if (targetFormat.equals(JPEG)){
				f = new File(output.getAbsolutePath()+"\\"+inputName.getName()+"-"+1.0+"."+targetFormat);
			}
			else{
				f = new File(output.getAbsolutePath()+"\\"+inputName.getName()+"."+targetFormat);
			}
			
			ImageIO.write(input, targetFormat, f);
	 ...
	 * @return
	 */
	public File writeJpegImage(BufferedImage input, File inputName, File output, float quality){
		File f = null;
		try{
			f = new File(output.getAbsolutePath()+"\\"+inputName.getName()+"-"+quality+".jpeg");
			...
			System.out.println("JPEG Writing Error!");
		}
		return f;
	}
    ...
    */
    public static void main( String[] args ) throws Exception
    {
        ...
        // NOTE: I set the parameters in my Run Configurations to test the program!
