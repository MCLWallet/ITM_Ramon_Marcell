...
public class ImageMetadataGenerator 
{
    ...
    */
    protected ImageMedia processImage( File input, File output, boolean overwrite ) throws IOException, IllegalArgumentException
    {
        ...
        // load the input image ( is already implemented in ImageMedia)
        
        
        // set width and height of the image  (see Constructor in ImageMedia)

        // add a tag "image" to the media
        media.addTag("image");
        // add a tag corresponding to the filename extension of the file to the media 
        media.addTag(fileName.substring(fileName.lastIndexOf(".")+1));
        
        // set orientation (see Constructor in ImageMedia)

        // if there is a colormodel: (see Constructor in ImageMedia)
        // set color space type
        // set pixel size
        // set transparency
        // set number of (color) components (see Constructor in ImageMedia)       

        // store meta data
        media.serializeObject();
        File newFile= new File(output+"\\"+fileName+".txt");
        media.writeToFile(newFile);
